...
```vue demo title="基础按钮" description="最基础的按钮组件，支持不同状态"
<template>
...
```vue demo title="输入框" description="支持多种类型的输入框"
<template>
...
```vue demo title="信息卡片" description="常用的卡片布局组件"
<template>
  <div class="card">
...
```vue demo title="标签" description="用于标记和分类的小型标签"
<template>
  <div class="tag-group">
...
```vue demo title="Alert 提示框" description="用于展示重要信息的提示组件"
<template>
...
```vue demo title="徽章 Badge" description="用于状态标记或数量提示"
<template>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
      <button 
        ...
        style={{ width: '32px', height: '32px', borderRadius: '4px', border: '1px solid #e5e7eb', background: 'white', cursor: 'pointer' }}
      >-</button>
      <span>{count}</span>
      <button 
        onClick={() => setCount(c => c + 1)}
