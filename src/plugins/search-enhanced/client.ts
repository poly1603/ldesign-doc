...
export const LDocEnhancedSearch = defineComponent({
  name: 'LDocEnhancedSearch',
  props: {
    ...
    fuzzy: { type: Object, default: () => ({ enabled: true, threshold: 0.6, distance: 2 }) },
    cjk: { type: Object, default: () => ({ enabled: true, segmenter: 'jieba' }) },
    filters: { type: Array, default: () => [] },
    suggestions: { type: Object, default: () => ({ enabled: true, maxSuggestions: 5 }) },
    history: { type: Object, default: () => ({ enabled: true, maxItems: 10, storageKey: 'ldoc-search-history' }) }
  },
  setup(props) {
    const open = () => {
    ...
    return () => h('button', {
      type: 'button',
      class: 'ldoc-search-enhanced-button',
      onClick: open,
...
export const globalComponents = [
  { name: 'LDocEnhancedSearch', component: LDocEnhancedSearch }
]
